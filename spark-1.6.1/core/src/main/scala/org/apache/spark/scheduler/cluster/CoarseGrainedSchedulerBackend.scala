    ...
    override def receive: PartialFunction[Any, Unit] = {
      case StatusUpdate(executorId, taskId, state, data) =>
        ...
        if (TaskState.isFinished(state)) {
          logInfo("Woyy!! DANIAR: in Coarse grained Scheduler TaskState.isFinished "+state)
...
//      logInfo("DANIAR -- MAKE OFFERS TO all SLAVES" )
    ...
    private def makeOffers(executorId: String) {
      ...
      logInfo("daniarrr -- makeOffers" + executorId)
...
//          logInfo("Daniarrr:  Before assigning any tasks it should be no random assignment [should be printed once]" )
    ...
    def removeExecutor(executorId: String, reason: ExecutorLossReason): Unit = {
      executorDataMap.get(executorId) match {
        case Some(executorInfo) =>
          ...
          totalRegisteredExecutors.addAndGet(-1)
          logInfo("in CoarseGrained Scheduler removeExecutor executorLOST TaskSchedulerImpl CALLED!!!")
