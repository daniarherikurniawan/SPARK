...
private[spark] class TaskSetManager(
  ...
  private def hasAttemptOnHost(taskIndex: Int, host: String): Boolean = {
    //logInfo("martin -- hasAttemptOnHost val " + taskAttempts(taskIndex).exists(_.host == host))
    ...
    : Option[(Int, TaskLocality.Value)] =
  {
    ...
    //logInfo("martin -- dequeue specX list " + speculatableTasks.size) //<--executed

    def canRunOnHost(index: Int): Boolean ={
      //logInfo("martin -- canRunOnHost.. index " + index + " host " + host + " execID " + execId + " locality " + locality)
    ...
    if (!speculatableTasks.isEmpty) {
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.NODE_LOCAL)) {
        for (index <- speculatableTasks if canRunOnHost(index)) {
          logInfo("martin -- spec2a")
          val locations = tasks(index).preferredLocations.map(_.host)
          if (locations.contains(host)) {
            logInfo("martin -- spec2b")
            speculatableTasks -= index
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.NO_PREF)) {
        for (index <- speculatableTasks if canRunOnHost(index)) {
          logInfo("martin -- spec3a")
          val locations = tasks(index).preferredLocations
          if (locations.size == 0) {
            logInfo("martin -- spec3b")
            speculatableTasks -= index
            return Some((index, TaskLocality.PROCESS_LOCAL))
          }
        }
      ...
      if (TaskLocality.isAllowed(locality, TaskLocality.RACK_LOCAL)) {
        for (rack <- sched.getRackForHost(host)) {
          for (index <- speculatableTasks if canRunOnHost(index)) {
            logInfo("martin -- spec4a")
            val racks = tasks(index).preferredLocations.map(_.host).map(sched.getRackForHost)
            if (racks.contains(rack)) {
              logInfo("martin -- spec4b")
              speculatableTasks -= index
    ...
    : Option[(Int, TaskLocality.Value, Boolean)] =
  {
    ...
    //logInfo("martin -- before dequeue specX list") <-- executed
    ...
    : Option[TaskDescription] =
  {
    if (!isZombie) {
      ...
      //logInfo("martin -- before dequeue Task for executor " + execId)
      dequeueTask(execId, host, allowedLocality) match {
        // martin: check this
        case Some((index, taskLocality, speculative)) => {
          logInfo("martin -- some1 host " + host + " spec " + speculative + " index " + index + " taskLocality " + taskLocality + " execId " + execId) //martin: speculative is a boolean value
          ...
          logInfo("martin -- task is started to execId " + execId)
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]): Unit = {
    ...
    logInfo("martin -- handle successful task for idx " + index)
    info.markSuccessful()
  ...
  override def checkSpeculatableTasks(): Boolean = {
    ...
    // logInfo("martin -- Checking for speculative tasks: minFinished = " + minFinishedForSpeculation) <- executed
    if (tasksSuccessful >= minFinishedForSpeculation && tasksSuccessful > 0) {
      ...
      //logInfo("martin -- Task length threshold for speculation: " + threshold) < -- executed
