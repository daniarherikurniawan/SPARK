  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receive: PartialFunction[Any, Unit] = {
        ...
        if (TaskState.isFinished(state)) {
          executorDataMap.get(executorId) match {
            case Some(executorInfo) =>
              ...
              logInfo("martin -- before makeOffers in Some")
      ...
      case ReviveOffers =>
        //logInfo("martin -- before makeOffers in ReviveOffers")
        makeOffers()
      ...
      case RegisterExecutor(executorId, executorRef, hostPort, cores, logUrls) =>
        if (executorDataMap.contains(executorId)) {
          ...
          val executorAddress = if (executorRef.address != null) {
              //logInfo("martin -- execAdr not null") //<-- not executed
              executorRef.address
            } else {
              //logInfo("martin -- execAdr is null") //<-- executed
              context.senderAddress
            }
          //logInfo("martin -- exec address " + executorRef.address)
          logInfo(s"Registered executor $executorRef ($executorAddress) with ID $executorId")
          // martin -- slowing based on port to disrupt the communication between driver and executor
          /*val srccmd: String = "sudo ipfw add 1 pipe 1 src-port " + executorAddress.toString.split(":").last
          val dstcmd: String = "sudo ipfw add 1 pipe 1 dst-port " + executorAddress.toString.split(":").last
          if (executorId.toInt == 0){
            "sudo ipfw pipe 1 config bw 10Kbits/s".!
            srccmd .!
            dstcmd .!
          }*/
          //end martin
          ...
          logInfo("martin -- before makeOffers in Receive and Reply")
          makeOffers()
        }
    ...
    private def makeOffers() {
      //logInfo("martin -- makeOffers to all") //executed so many times
      // Filter out executors under killing
      val activeExecutors = executorDataMap.filterKeys(executorIsAlive)
      val workOffers = activeExecutors.map { case (id, executorData) =>
        new WorkerOffer(id, executorData.executorHost, executorData.freeCores)
      }.toSeq
      //martin: executorId is not yet defined
      launchTasks(scheduler.resourceOffers(workOffers))
      //martin: now it has gotten the executor Id
    }
    ...
    private def makeOffers(executorId: String) {
      logInfo("martin -- makeOffers to execId " + executorId)
      // Filter out executors under killing
    ...
    //martin: where resource offers are send to executors
