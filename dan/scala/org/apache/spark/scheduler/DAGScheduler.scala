...
class DAGScheduler(
  ...
  def submitJob[T, U](
    ...
    logInfo("martin -- submit job method in DAGScheduler")
    eventProcessLoop.post(JobSubmitted(
      jobId, rdd, func2, partitions.toArray, callSite, waiter,
      SerializationUtils.clone(properties)))
    //logInfo("martin -- end of submitJob") //executed
    waiter
  }
  ...
  def runJob[T, U](
    ...
    logInfo("martin -- submit job in runJob")
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    clearCacheLocs()
    //martin: "partitions" length is from minpartition..?
    ...
    //logInfo("martin -- before waiting stages")
    submitWaitingStages()
    //logInfo("martin -- after waiting stages")
  }
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      //logInfo("martin -- submitStage(" + stage + ")")
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    if (tasks.size > 0) {
      ...
      //martin: flag -- submiTasks by DAGScheduler
