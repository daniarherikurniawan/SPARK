...
abstract class RDD[T: ClassTag](
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    if (storageLevel != StorageLevel.NONE) {
      logInfo(s"martin -- rdd 1 cacheManager split" + split.index)
      SparkEnv.get.cacheManager.getOrCompute(this, split, context, storageLevel)
    } else {
      logInfo(s"martin -- rdd 1 computeOrReadCheckpount split " + split.index)
  ...
  private[spark] def computeOrReadCheckpoint(split: Partition, context: TaskContext): Iterator[T] =
  {
    if (isCheckpointedAndMaterialized) {
      logInfo(s"martin -- rdd 3 checkpointedandmaterialized " + split)
      firstParent[T].iterator(split, context)
    } else {
      logInfo(s"martin -- rdd 3 compute " + split.index)
  ...
  def mapPartitions[U: ClassTag](
    ...
    logInfo(s"martin -- rdd mappartitions")
  ...
  private[spark] def mapPartitionsInternal[U: ClassTag](
      f: Iterator[T] => Iterator[U],
      preservesPartitioning: Boolean = false): RDD[U] = withScope {
    logInfo(s"martin -- rdd mappartitions internal")
    new MapPartitionsRDD(
      this,
  ...
  def mapPartitionsWithIndex[U: ClassTag](
      ...
      preservesPartitioning: Boolean = false): RDD[U] = withScope {
    logInfo(s"martin -- rdd mappartitions with idx")
    val cleanedF = sc.clean(f)
    new MapPartitionsRDD(
      this,
  ...
  def mapPartitionsWithContext[U: ClassTag](
      ...
      preservesPartitioning: Boolean = false): RDD[U] = withScope {
    logInfo(s"martin -- rdd mappartitions with context")
    val cleanF = sc.clean(f)
  ...
  def collect(): Array[T] = withScope {
    ...
    logInfo("martin -- martin-flag collect " + results.deep.mkString(";"))
