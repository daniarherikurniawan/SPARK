...
class HadoopRDD[K, V](
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    val iter = new NextIterator[(K, V)] {
      ...
      //logInfo("martin -- Input split value : " + split.inputSplit.value) //same as above
      val jobConf = getJobConf()
      ...
      override def getNext(): (K, V) = {
        try {
          //logInfo("martin -- key " + key + " value " + value) // printed
...
private[spark] object HadoopRDD extends Logging {
  ...
  private[spark] class HadoopMapPartitionsWithSplitRDD[U: ClassTag, T: ClassTag](
    ...
    override def compute(split: Partition, context: TaskContext): Iterator[U] = {
      ...
      //logInfo("martin -- input split value " + inputSplit)
