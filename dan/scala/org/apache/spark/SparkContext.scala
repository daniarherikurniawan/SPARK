...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  def textFile(
    ...
    logInfo("martin -- read from path " + path + " minpartitions " + minPartitions)
    assertNotStopped()
  ...
  def hadoopFile[K, V](
      ...
      minPartitions: Int = defaultMinPartitions): RDD[(K, V)] = withScope {
    assertNotStopped()
    logInfo("martin -- martin-flag broadcast hadoop configuration file: " + hadoopConfiguration)
  ...
  def runJob[T, U: ClassTag](
    ...
    logInfo("martin -- rdd " + rdd.toDebugString +  " func " + cleanedFunc.toString() + " partitions " + partitions.toString())
    dagScheduler.runJob(rdd, cleanedFunc, partitions, callSite, resultHandler, localProperties.get)
    //logInfo("martin -- after run job") <-- executed
  ...
  def runJob[T, U: ClassTag](
      ...
      partitions: Seq[Int]): Array[U] = {
    val cleanedFunc = clean(func)
    logInfo("martin -- martin-flag rdd: \n " + rdd.toDebugString)
  ...
  def runJob[T, U: ClassTag](
    ...
    logInfo("martin -- runJob before 1-2")
  ...
  def runJob[T, U: ClassTag](rdd: RDD[T], func: (TaskContext, Iterator[T]) => U): Array[U] = {
    //logInfo("martin -- runJob before 1-3")
   ...
   * Run a job on all partitions in an RDD and return the results in an array.
   */
  //martin: this is called by
  def runJob[T, U: ClassTag](rdd: RDD[T], func: Iterator[T] => U): Array[U] = {
    logInfo("martin -- martin-flag runJob")
    runJob(rdd, func, 0 until rdd.partitions.length)
  }
  ...
  {
    logInfo("martin -- submit job and call DAGScheduler")
    assertNotStopped()
