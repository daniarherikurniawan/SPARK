...
private[spark] class Executor(
  ...
  logInfo(s"martin -- Before Launch task")
  def launchTask(
    ...
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)
    logInfo(s"martin -- Launch task for " + taskId)
    //start - martin
    /*if(taskId == 5) {
      Thread.sleep(60000);
    }*/
  ...
  class TaskRunner(
    ...
    logInfo(s"martin -- Task Runner start")
    ...
    override def run(): Unit = {
      logInfo(s"martin -- Executor.scala , run task in Task Runner")
      ...
      try {
        ...
        logInfo("martin -- direct result " + directResult.value()) //???
