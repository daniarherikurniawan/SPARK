...
object SparkEnv extends Logging {
  ...
  private[spark] def createExecutorEnv(
    ...
    //logInfo("martin -- sss port " + port) port = 0
    val env = create(
      conf,
      executorId,
      hostname,
  ...
  private def create(
    ...
    val actorSystem: ActorSystem =
      if (rpcEnv.isInstanceOf[AkkaRpcEnv]) {
        ...
        logInfo("martin -- actor port " +  actorSystemPort)
        AkkaUtils.createActorSystem(
          ...
          conf,
          securityManager
        )._1 //martin: this should return the port
      }
    //logInfo("martin -- actorSystem " + actorSystem.name + " rpcEnv address port " + rpcEnv.address.port.toString)
    ...
    if (isDriver) {
      logInfo("martin: isDriver port " + rpcEnv.address.port.toString)
      conf.set("spark.driver.port", rpcEnv.address.port.toString)
    } else if (rpcEnv.address != null) {
      logInfo("martin: isExecutor port " + rpcEnv.address.port.toString)
